...
state: Dict[str, Any] = {
    ...
    "greens_gale_1": 0,
    "greens_gale_2": 0,
    "total_empates": 0,           # só empates FORA de sinal ativo
    "total_losses": 0,
...
# ───────────────────────────────────────────────
# PADRÕES (mantidos iguais – pode ajustar depois)
# ───────────────────────────────────────────────

PATTERNS = [
    # ... (coloque aqui todos os padrões que estavam antes – não alterei)
    # Exemplo resumido:
...
async def update_history_from_api(session):
    ...
    try:
        ...
        if outcome and state["last_round_id"] != rid:
            ...
            # Capturar somas (player/banker score)
            result = data.get("result", {})
            ps = bs = None
            # ... (lógica de extração de ps/bs mantida igual à original)
            ...
            # Só conta empate geral se NÃO estiver em sinal ativo
...
# ───────────────────────────────────────────────
# PROCESSAMENTO DE RESULTADO (principal mudança aqui)
# ───────────────────────────────────────────────
...
async def process_round_result():
    ...
    if ultimo == alvo or ultimo == "🟡":
        ...
        gale_txt = "" if state["martingale_count"] == 0 else f" (gale {state['martingale_count']})"

        await send_to_channel(
            f"✅ <b>GREEN{gale_txt}{extra}</b> ✅\n"
            f"{'Entrada principal' if state['martingale_count'] == 0 else f'Gale {state['martingale_count']}'}\n"
        )

        p = state.get("player_score_last", "?")
        b = state.get("banker_score_last", "?")
        await send_to_channel(f"💰 {alvo} | {p} • {b}  {'🟡' if ultimo == '🟡' else ultimo}")
    ...
    # Perdeu (não acertou nem empatou)
...
async def main():
    logger.info("Bot iniciado – Empate agora conta como GREEN")
